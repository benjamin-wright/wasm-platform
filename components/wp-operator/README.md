...
| `spec.kv` | KVSpec | no | Key-value store access. When present (even as `kv: {}`), the operator provisions a cluster-wide `RedisDatabase` and a `RedisCredential` for the execution host if they do not already exist. When absent, no KV access is provisioned. |
