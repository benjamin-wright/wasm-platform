...
|---|---|---|
| **Execution Host** | `components/execution-host/` | Rust binary — syncs config from the wp-operator via gRPC (`RequestFullConfig` on startup; `PushIncrementalUpdate` for ongoing deltas), checks the module cache, pulls and AOT-compiles WASM modules on a cache miss, exposes an HTTP endpoint, and calls guest exports. |
| **WP Operator** | `components/wp-operator/` | Go operator — watches `Application` CRDs, reconciles database bindings and message subscriptions, and syncs config to execution hosts via a gRPC `ConfigSync` service (`RequestFullConfig` + `PushIncrementalUpdate`). |
